    ...
    for i, path in enumerate(paths):
        for txtfile in os.listdir(path):
            ...
            #artcile is a list of words in article after preprocessing
                with open(path+'/'+txtfile, 'r') as myfile:
                    article=myfile.read()
...
def generate_word_features_set(word_features_file):
    ...
    for i in word_features_set:
        single_word_features=i.split()
        single_word_features_set+=[w for w in single_word_features if (not w in stopwords.words('english'))  and (not w.isdigit())]
...
tags=["pos","neg"]
word_features_file="/Users/macbook/Documents/RA/classicifation/Syndrom_Prescursor_Control_Agent_01:10:2017/variation_Syndrom_Prescursor_Control_Agent.txt"
